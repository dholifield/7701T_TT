...
void initscr() {
  ...
  // text
  lv_obj_t *init_title = lv_label_create(scr, NULL);
  lv_obj_set_style(init_title, &title_style);
  lv_label_set_text(init_title, "INITIALIZING");
  lv_obj_align(init_title, NULL, LV_ALIGN_CENTER, 0, 0);
}
...
void display() {  // tesseract gif with tesseract text at bottom runs during opcontrol
  ...
  // static Gif gif("/usd/tesseractres.gif", lv_scr_act());
...
void redbig1scr() {
  ...
  static lv_style_t background_style;
  lv_style_copy(&background_style, &lv_style_plain);
  background_style.body.main_color = LV_COLOR_PURPLE;
  background_style.body.grad_color = LV_COLOR_PURPLE;
